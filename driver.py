...
def cfg():
    ...
    lr = random.choice(np.logspace(-2,0,num=20))
...
def set_up_data_loader(_config):
    ...
    base_path="/scratch/mhasan8/UR_LYING/Mental_face_transfer_data/"
    ...
    openface_file=base_path+"relevant_openface2_sdk.pkl"
    affectiva_file=base_path+"relevant_affectiva_sdk.pkl"
    covarep_file=base_path+"relevant_covarep_sdk.pkl"
    ...
    all_train_x,all_train_y=get_data(train_roots,openface_sdk,affectiva_sdk,relevant_pretrained_features,relevant_data_property)
    test_x,test_y=get_data(test_roots,openface_sdk,affectiva_sdk,relevant_pretrained_features,relevant_data_property)
...
def test_epoch(model,data_loader,criterion, device,_config):
    ...
    with torch.no_grad():
        ...
        for batch in tqdm(data_loader, mininterval=2,desc='  - (Test)   ', leave=False):
            ...
            returned_predictions=predictions.cpu().data.numpy()
...
def test_score_from_file(test_data_loader,criterion,_config,_run):
    ...
    predicted_label = torch.round(torch.sigmoid(torch.FloatTensor(predictions)))
    ...
    _run.info['final_result']={'accuracy':accuracy,
             "f_score":f_score,"Confusion Matrix":confusion_matrix_result,"sum_scalar":model.sum_scalar.cpu().data.numpy()[0],"pre_scalar":model.pre_scalar.cpu().data.numpy()[0]}
    return accuracy
